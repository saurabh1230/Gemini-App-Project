...
class BasicNotesDashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      notesController.getBasicPaginatedList("1", categoryId!);
    ...
    // Add listener in build
    _loopPageController.addListener(() async {
      final BasicController controller = Get.find();
      ...
      // Only use the ScrollController if it has a position
      ...
      if (pageIndex == controller.categoryNoteList!.length - 1 && !controller.isBasicLoading) {
                    ...
                    child: LoopPageView.builder(
                      ...
                      itemBuilder: (context, i) {
                        return GetBuilder<BookmarkController>(builder: (bookmarkControl) {
                          bool isBookmarked = bookmarkControl.bookmarkBasicIdList.contains(noteList![i].id);
                          return NotesViewComponent(
                            title: noteList[i].title.toString(),
                            question: noteList[i].content.toString(),
                            saveNote: () {
                              isBookmarked
                                  ? bookmarkControl.removeBasicBookMarkList(int.parse(noteList[i].id.toString()))
                                  : bookmarkControl.addBasicBookMarkList('', noteList[i]);
                            },
                            saveNoteColor: isBookmarked ? Theme.of(context).cardColor : Theme.of(context).cardColor.withOpacity(0.60),
                          );
                        });
                      },
                    ),
